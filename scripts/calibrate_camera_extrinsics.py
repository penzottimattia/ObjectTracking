...
def main():
    ...
    from utils.camera_extrinsics import (
        RelativeCameraCalibrator,
        save_consensus_yaml,
    )
    ...
    capture = MultiCameraCapture(
        args.cameras, args.width, args.height, args.fps
    )
    ...
    def restart_acquisition(reason):
        nonlocal last_attempt
        removed = calibrator.clear_samples()
        ...
        try:
            ...
            pass
        logging.warning(
            "Calibration acquisition restarted (%s); discarded %d sample(s). "
            "Move the probe only after all cameras register it correctly.",
            reason,
            removed,
        )

    logging.info(
        "Move the probe through diverse positions and orientations visible to every camera."
    )
    logging.info(
        "Press r whenever registration or tracking looks wrong. "
        "All collected samples will be discarded."
    )
    ...
    try:
            ...
            if not all(state["initialized"] for state in states):
                ...
                try:
                    track_batch(
                        refiner,
                        glctx,
                        collect_tracking_units(tracked, frames),
                        args.track_refine_iter,
                    )
                except Exception as exc:
                    logging.warning("Tracking failed: %s", exc)
                    restart_acquisition("tracking failure")
                    continue
                ...
                if now - last_attempt >= args.sample_period:
                    ...
                    if calibrator.add_sample(camera_poses):
                        logging.info(
                            "Accepted calibration sample %d/%d",
                            len(calibrator.samples),
                            args.samples,
                        )

            if display:
                key = display.show(
                    draw_multi_camera_vis(frames, tracked, 0.0)
                )
                if key in ("q", "Escape"):
                    raise KeyboardInterrupt
                if key == "r":
                    restart_acquisition("manual reset")
                    continue
        ...
        for camera in args.cameras:
            ...
            logging.info(
                "%s: xyz=(%.6f, %.6f, %.6f), translation RMSE=%.4f m, "
                "rotation RMSE=%.3f deg",
                camera,
                pose[0, 3],
                pose[1, 3],
                pose[2, 3],
                result.translation_rmse_m[camera],
                result.rotation_rmse_deg[camera],
            )
    except KeyboardInterrupt:
        logging.info(
            "Calibration cancelled after %d accepted samples",
            len(calibrator.samples),
        )
    finally:
        capture.stop()
